...
namespace thinger.DataConvertLib
{
    ...
    public class StringLib
    {
        ...
        {
            byte[] data = ByteArrayLib.GetByteArrayFromByteArray(source, start, length + 2);

            int valid = data[1];

            if (valid > 0)
            {
                return Encoding.GetEncoding("GBK").GetString(ByteArrayLib.GetByteArrayFromByteArray(data, 2, valid));
            }
            else
            {
                return emptyStr;
